...
export type AssetLoanRecord = { id:string; requesterUserId:string; requesterName:string; requesterDepartment:string; projectId:string; project:string; itemName:string; purpose:string; neededFrom:string; expectedReturnDate:string; fulfillmentSource:string; assetId:string|null; assetName:string; purchaseRequestId:string|null; status:string; reviewNote:string; issuedAt:string|null; issuedByName:string; returnedAt:string|null; returnedByName:string; returnCondition:string; createdAt:string; updatedAt:string; events:{id:number;eventType:string;fromStatus:string;toStatus:string;note:string;actorName:string;createdAt:string}[] };
...
export async function createPurchaseOrder(input:Row){const session=await createClient();const{data:{user}}=await session.auth.getUser();const supabase=createAdminClient();const equipmentType=text(input.equipmentType)==="Consumable"?"Consumable":"Asset";const quantity=Math.max(1,num(input.quantity,1)),unitPrice=num(input.unitPrice);const id=text(input.id)||`IBT-PO-${Date.now().toString().slice(-7)}`,projectId=await projectIdByName(text(input.project));const{data,error}=await supabase.from("purchase_orders").insert({id,project_id:projectId,po_date:text(input.date)||new Date().toISOString().slice(0,10),supplier:text(input.supplier,"To be confirmed"),equipment:text(input.equipment),equipment_type:equipmentType,budget_branch:text(input.budgetBranch,equipmentType==="Asset"?"Equipment":"Consumables"),location:text(input.location),quantity,unit_price:unitPrice,status:"Review",purchase_request_id:text(input.purchaseRequestId)||null,created_by:user?.id||null}).select("*,projects(name),approved_by_profile:profiles!purchase_orders_approved_by_fkey(full_name)").single();fail(error);await insertNotifications(await recipientsForRoles(["ADMIN"]),"purchase_order",id,"Purchase order ready for completion",`${id} · ${text(input.equipment)} · ${text(input.project)} · enter the supplier, final price and expected delivery date.`);await recalculateProjectFinancials(projectId);return poFromRow(data as Row);}
...
async function createPurchaseOrderFromApprovedRequest(request:ReturnType<typeof prFromRow>){
  ...
  await syncOrderProgressByRequest(request.id,{purchaseOrderId:order.id,progressPercentage:10,status:"PO Pending",supplier:order.supplier});
...
function loanFromRow(row:Row,events:Row[]=[]):AssetLoanRecord{return{id:text(row.id),requesterUserId:text(row.requester_user_id),requesterName:relName(row.requester),requesterDepartment:text(oneRelation(row.requester)?.department),projectId:text(row.project_id),project:relName(row.projects),itemName:text(row.item_name),purpose:text(row.purpose),neededFrom:text(row.needed_from),expectedReturnDate:text(row.expected_return_date),fulfillmentSource:text(row.fulfillment_source),assetId:row.asset_id?text(row.asset_id):null,assetName:text(oneRelation(row.assets)?.name),purchaseRequestId:row.purchase_request_id?text(row.purchase_request_id):null,status:text(row.status),reviewNote:text(row.review_note),issuedAt:row.issued_at?text(row.issued_at):null,issuedByName:relName(row.issued_by_profile),returnedAt:row.returned_at?text(row.returned_at):null,returnedByName:relName(row.returned_by_profile),returnCondition:text(row.return_condition),createdAt:text(row.created_at),updatedAt:text(row.updated_at),events:events.map(event=>({id:num(event.id),eventType:text(event.event_type),fromStatus:text(event.from_status),toStatus:text(event.to_status),note:text(event.note),actorName:relName(event.actor),createdAt:text(event.created_at)}))};}
const loanSelect="*,requester:profiles!asset_loan_requests_requester_user_id_fkey(full_name,department),projects(name),assets(name),issued_by_profile:profiles!asset_loan_requests_issued_by_fkey(full_name),returned_by_profile:profiles!asset_loan_requests_returned_by_fkey(full_name)";
export async function listAssetLoans(){const supabase=await createClient();const{data,error}=await supabase.from("asset_loan_requests").select(loanSelect).order("created_at",{ascending:false});fail(error);return(data||[]).map(row=>loanFromRow(row as Row));}
export async function getAssetLoan(id:string){const supabase=await createClient();const[{data,error},history]=await Promise.all([supabase.from("asset_loan_requests").select(loanSelect).eq("id",id).maybeSingle(),supabase.from("asset_loan_events").select("*,actor:profiles!asset_loan_events_actor_user_id_fkey(full_name)").eq("loan_request_id",id).order("created_at",{ascending:false})]);fail(error);fail(history.error);return data?loanFromRow(data as Row,(history.data||[]) as Row[]):undefined;}
async function assetLoanStakeholders(requester:Pick<UserRecord,"id"|"department">,projectId:string){const admin=createAdminClient();const[{data:people,error:peopleError},{data:access,error:accessError}]=await Promise.all([admin.from("profiles").select("id,department,roles!inner(name)").eq("active",true).in("roles.name",["ADMIN","MANAGER","PROJECT_MANAGER"]),admin.from("user_project_access").select("user_id").eq("project_id",projectId).eq("can_view",true)]);fail(peopleError);fail(accessError);const projectUsers=new Set((access||[]).map(row=>text(row.user_id))),department=requester.department.trim().toLowerCase();return[...new Set((people||[]).filter(row=>{const role=text(oneRelation(row.roles)?.name);if(role==="ADMIN")return true;return Boolean((department&&text(row.department).trim().toLowerCase()===department)||projectUsers.has(text(row.id)));}).map(row=>text(row.id)).filter(id=>id&&id!==requester.id))];}
export async function createAssetLoan(input:Row,actor:UserRecord){const itemName=text(input.itemName).trim(),purpose=text(input.purpose).trim(),neededFrom=text(input.neededFrom),expectedReturnDate=text(input.expectedReturnDate);if(!itemName||!purpose||!neededFrom||!expectedReturnDate)throw new Error("Item, purpose, needed date, and return date are required");if(expectedReturnDate<neededFrom)throw new Error("Expected return date cannot be before the needed date");const projectId=await projectIdByName(text(input.project));const id=`IBT-LOAN-${Date.now().toString().slice(-7)}`,supabase=await createClient();const{data,error}=await supabase.from("asset_loan_requests").insert({id,requester_user_id:actor.id,project_id:projectId,item_name:itemName,purpose,needed_from:neededFrom,expected_return_date:expectedReturnDate}).select(loanSelect).single();fail(error);fail((await supabase.from("asset_loan_events").insert({loan_request_id:id,actor_user_id:actor.id,event_type:"Requested",to_status:"Submitted",note:purpose})).error);await insertNotifications(await assetLoanStakeholders(actor,projectId),"asset_loan",id,"New asset loan request",`${actor.name} requested ${itemName} for ${text(input.project)} from ${neededFrom}. Review stock availability or start purchasing.`);return loanFromRow(data as Row);}
export async function updateAssetLoan(id:string,input:Row,actor:UserRecord){const current=await getAssetLoan(id);if(!current)throw new Error("Asset loan request not found");const action=text(input.action).toLowerCase(),supabase=await createClient();let status=current.status,eventType="Updated",note=text(input.note).trim(),patch:Row={};
 if(action==="assign_stock"||action==="ready"){const assetId=text(input.assetId).trim();if(!assetId)throw new Error("Choose an available asset");const asset=await findAsset(assetId);if(!asset)throw new Error("Asset not found");const{data:conflict,error:conflictError}=await supabase.from("asset_loan_requests").select("id").eq("asset_id",assetId).eq("status","On loan").neq("id",id).maybeSingle();fail(conflictError);if(conflict)throw new Error("This asset is already on loan");status="Ready for delivery";eventType=action==="assign_stock"?"Stock allocated":"Purchased asset allocated";patch={asset_id:assetId,fulfillment_source:action==="assign_stock"?"Stock":"Purchase",review_note:note};}
 else if(action==="purchase"){const amount=num(input.amount);if(amount<=0)throw new Error("Estimated purchase value is required");const pr=await createPurchaseRequest({project:current.project,fundingProject:current.project,itemName:current.itemName,reason:"New equipment / asset",budgetBranch:"Equipment",amount,description:`Asset loan ${id}: ${current.purpose}`,requester:actor.name});status="Awaiting purchase";eventType="Sent to purchasing";patch={fulfillment_source:"Purchase",purchase_request_id:pr.id,review_note:note};}
 else if(action==="issue"){if(current.status!=="Ready for delivery"||!current.assetId)throw new Error("Allocate an asset before issuing the loan");status="On loan";eventType="Issued";patch={issued_at:new Date().toISOString(),issued_by:actor.id};fail((await supabase.from("assets").update({status:"On loan",tone:"due",owner:current.requesterName}).eq("id",current.assetId)).error);}
 else if(action==="return"){if(current.status!=="On loan"||!current.assetId)throw new Error("Only an issued loan can be returned");const condition=text(input.returnCondition).trim();if(!condition)throw new Error("Record the return condition");status="Returned";eventType="Returned";note=condition;patch={returned_at:new Date().toISOString(),returned_by:actor.id,return_condition:condition};fail((await supabase.from("assets").update({status:"Operational",tone:"healthy"}).eq("id",current.assetId)).error);}
 else if(action==="reject"){status="Rejected";eventType="Rejected";patch={review_note:note};}
 else if(action==="cancel"){if(current.requesterUserId!==actor.id&&!['ADMIN','MANAGER'].includes(actor.role))throw new Error("Only the requester or a manager can cancel this request");status="Cancelled";eventType="Cancelled";}
 else throw new Error("Unknown asset loan action");
 const{error}=await supabase.from("asset_loan_requests").update({...patch,status}).eq("id",id);fail(error);fail((await supabase.from("asset_loan_events").insert({loan_request_id:id,actor_user_id:actor.id,event_type:eventType,from_status:current.status,to_status:status,note})).error);const requester=await findUserById(current.requesterUserId),stakeholders=requester?await assetLoanStakeholders(requester,current.projectId):[];await insertNotifications([...stakeholders,current.requesterUserId].filter(userId=>userId!==actor.id),"asset_loan",id,`Asset loan ${status}`,`${current.itemName} · ${eventType}${note?` · ${note}`:""}`);return getAssetLoan(id);}

function notificationLink(type:string,id:string){if(type.includes("work_order"))return`/work-orders?open=${encodeURIComponent(id)}`;if(type==="purchase_order")return`/procurement?open=${encodeURIComponent(id)}`;if(type==="purchase_request")return`/purchase-requests?open=${encodeURIComponent(id)}`;if(type==="asset_loan")return`/asset-loans?open=${encodeURIComponent(id)}`;if(type==="payment_request")return`/finance/payments?open=${encodeURIComponent(id)}`;if(type==="budget_request")return`/project-budgets?open=${encodeURIComponent(id)}`;if(type==="customer_invoice_request")return`/finance/invoicing?open=${encodeURIComponent(id)}`;if(type==="order_progress")return`/order-progress?open=${encodeURIComponent(id)}`;if(type==="project_task")return`/tasks?open=${encodeURIComponent(id)}`;if(type==="asset")return`/assets/${encodeURIComponent(id)}`;return"/";}
